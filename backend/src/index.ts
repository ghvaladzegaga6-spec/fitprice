...
app.options('*', cors());
const globalLimiter = rateLimit({ windowMs:15*60*1000, max:300, standardHeaders:true, legacyHeaders:false, message:{error:'ძალიან ბევრი მოთხოვნა.'} });
const authLimiter = rateLimit({ windowMs:15*60*1000, max:50, message:{error:'ძალიან ბევრი მოცდილობა.'} });
