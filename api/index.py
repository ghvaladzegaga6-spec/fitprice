...
genai.configure(api_key="შენი_API_KEY_აქ")
...
def calculate():
    try:
        ...
        # პრომოების დამუშავება ლიმიტებით
        selected_promos = data.get('selectedPromos', [])
        for promo in selected_promos:
            p_name = str(promo['product']).lower()
            is_liquid = any(w in p_name for w in ['წვენი', 'წყალი', 'რძე', 'კაკაო', 'სასმელი'])
            min_g, max_g = (250, 500) if is_liquid else (100, 300)
            
            u_w = clean_float(promo.get('unit_weight')) or clean_float(promo.get('total_package_weight')) or 100
            use_grams = max(min_g, min(max_g, u_w))

            final_items.append({
                "name": f"⭐ {promo['product']}",
                "display": f"პრომო (გამოიყენე {round(use_grams)}გ)",
                "cost": clean_float(promo['price'])
            })
            
            totals['p'] += (clean_float(promo['protein']) * use_grams) / 100
            totals['f'] += (clean_float(promo['fat']) * use_grams) / 100
            totals['c'] += (clean_float(promo['carbs']) * use_grams) / 100
            totals['cal'] += (clean_float(promo['calories']) * use_grams) / 100
        ...
        # Bounds დაწესება (100-300გ ან 250-500მლ)
        bounds = []
        for _, row in opt_df.iterrows():
            p_name = str(row['product']).lower()
            is_liquid = any(w in p_name for w in ['წვენი', 'წყალი', 'რძე', 'კაკაო', 'სასმელი'])
            bounds.append((0, 5.0 if is_liquid else 3.0))
            ...
            if res.success:
                for i, x in enumerate(res.x):
                    ...
                    if s_type == 'package_pieces' and u_w > 0:
                        ...
                        f_grams = count * u_w
                        if f_grams > 300: count = max(1, 300 // int(u_w)); f_grams = count * u_w
                        txt = f"იყიდე 1 შეკვრა (გამოიყენე {count} ცალი)"
                        cost = float(row['price'])
                    elif s_type == 'package_weight':
                        f_grams = min(300, grams)
                        txt = f"იყიდე 1 შეკვრა (გამოიყენე ~{round(f_grams)}გ)"
                        cost = float(row['price'])
                    else:
                        f_grams = max(100, min(300, grams))
...
def get_recipe():
    try:
        ...
        csv_path = os.path.join(BASE_DIR, '2nabiji.csv')
        df = pd.read_csv(csv_path)
        shop_list = df[['product', 'price', 'unit_weight']].to_string()

        prompt = f"""
        შენ ხარ ქართველი შეფ-მზარეული. მომხმარებელს აქვს ეს კალათა: {items_str}.
        
        1. თუ ამ ინგრედიენტებით (და პორციებით) შესაძლებელია კერძის მომზადება, დაწერე რეცეპტი.
        2. თუ არ არის საკმარისი, ნახე მაღაზიის სია:
        {shop_list}
        ამოარჩიე 1 ყველაზე იაფი პროდუქტი, რომელიც კერძს შეკრავს და უთხარი: "თუ დაამატებ [პროდუქტს] ([ფასი]₾), შეძლებ მოამზადო [კერძი]".
        3. რეცეპტი დაწერე მოკლედ და მეგობრულად ქართულ ენაზე.
        """
