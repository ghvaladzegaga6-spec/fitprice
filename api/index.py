...
def calculate():
    try:
        ...
        # ტიპების კონვერტაცია
        for col in ['protein', 'fat', 'carbs', 'calories', 'price', 'unit_weight', 'total_package_weight']:
        ...
        # --- 1. პრომოების დამატება (აუცილებლად აქ უნდა იყოს) ---
        selected_promos = data.get('selectedPromos', [])
        for promo in selected_promos:
            # ვადგენთ პრომოს რეალურ წონას მაკროების დასათვლელად
            weight = clean_float(promo.get('unit_weight')) if promo.get('sale_type') == 'package_pieces' else clean_float(promo.get('total_package_weight'))
            ...
            # ვამატებთ ჯამურ მაკროებს
            totals['p'] += (clean_float(promo['protein']) * weight) / 100
            totals['f'] += (clean_float(promo['fat']) * weight) / 100
            totals['c'] += (clean_float(promo['carbs']) * weight) / 100
            totals['cal'] += (clean_float(promo['calories']) * weight) / 100
