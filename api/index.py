...
def calculate():
    try:
        ...
        # --- 1. პრომოების გათვალისწინება (შენი ლოგიკით) ---
        selected_promos = data.get('selectedPromos', [])
        for promo in selected_promos:
            # ვადგენთ წონას მაკროების დასათვლელად
            p_weight = clean_float(promo.get('unit_weight')) if str(promo.get('sale_type')).strip().lower() == 'package_pieces' else clean_float(promo.get('total_package_weight'))
        ...
        # დარჩენილი მაკროები ოპტიმიზაციისთვის
...
def get_recipe():
    try:
        ...
        # ბაზიდან იაფი პროდუქტების წამოღება რეცეპტისთვის
        csv_path = os.path.join(BASE_DIR, '2nabiji.csv')
        df = pd.read_csv(csv_path)
        cheap_items = df.sort_values(by='price').head(12).to_dict(orient='records')

        prompt = f"""
        მომხმარებლის კალათა: {", ".join(basket_names)}.
        ბაზაში არსებული იაფი პროდუქტები დასამატებლად: {cheap_items}.
        
        დავალება:
        1. დაწერე რეცეპტი მხოლოდ კალათის პროდუქტებით, თუ შესაძლებელია.
        2. თუ პროდუქტები არასაკმარისია, შეარჩიე იაფი ინგრედიენტები სიიდან და დაწერე: "თუ შენ ამ პროდუქტებს შეიძენ, შეძლებ მოამზადო ეს:..."
        3. ბოლოში მკაფიოდ დათვალე დასამატებელი თანხა, კალორიები და მაკროები.
        დაწერე ქართულად, ძალიან მოკლედ.
        """
        ...
        return jsonify({"error": "შეცდომა რეცეპტისას"}), 500
